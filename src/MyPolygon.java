...
public class MyPolygon extends MyShape{
	...
	public MininumTranslationVector minimumTranslationVector(ArrayList<Vector> axes,MyCircle circle,Vector displacement){
		...
		return getMTV(this,m,displacement,axes);
	}
	public MininumTranslationVector minimumTranslationVector(ArrayList<Vector> axes,MyPolygon polygon,Vector displacement){
		return getMTV(this,polygon,displacement,axes);
	}
	
	public MininumTranslationVector getMTV(MyPolygon polygon,MyCircle circle, Vector displacement,ArrayList<Vector>axes) {
		   ...
		   for (int i=0; i < axes.size(); ++i) {
		      ...
		      Projection projection1 = polygon.project(axis);
		      Projection projection2 = m.project(axis);
	...
	public MininumTranslationVector getMTV(MyPolygon polygon,MyPolygon polygon1, Vector displacement,ArrayList<Vector>axes) {
		   float minimumOverlap = BIG_NUMBER;
		   float overlap;
		   Vector axisWithSmallestOverlap = new Vector();
		   MininumTranslationVector  mtv;
		   ...
		   for (int i=0; i < axes.size(); ++i) {
		      Vector axis = axes.get(i);
		      Projection projection1 = polygon.project(axis);
		      Projection projection2 = polygon1.project(axis);
		      overlap = projection1.getOverlap(projection2);
